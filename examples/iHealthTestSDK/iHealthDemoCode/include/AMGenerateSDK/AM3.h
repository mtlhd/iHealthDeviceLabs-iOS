 ...
 * @param authenticationResultBlock The return parameters of ’‘userid’, ’clientID’,and ‘clientSecret’ after verification.
 ...
 * @param currentUserIDBlock Uniquely identifies the user, the SDK requires this to be stored. This ID will be sent to the AM3 and will allow the AM3 to pair with only this user.
 * @param errorBlock Communication error codes, see AM3 error descriptions.
 */
-(void)commandAM3CreateUserManageConnectWithUser:(HealthUser *)tempUser withAuthenticationResult:(BlockUserAuthentication)authenticationResultBlock withCurrentUserID:(CurrentSerialNub)currentUserIDBlock withErrorBlock:(DisposeAM3ErrorBlock)errorBlock;

/**
 * Get device userID and user bined sevice MAC Address
 * @param getDeviceUserIDBlock The connected user's MAC Address
 * @param userBinedDeviceMACBlock The user's AM3 MAC Address
 * @param errorBlock Communication error codes, see AM3 error descriptions.
 */
-(void)commandAM3GetDeviceUserID:(DisposeAM3GetDeviceUserIDBlock)getDeviceUserIDBlock withFromCloudGetUserBinedDeviceMAC:(DisposeAM3GetUserBinedDeviceMACBlock)userBinedDeviceMACBlock withErrorBlock:(DisposeAM3ErrorBlock)errorBlock;
